...
**อ่าน `metadata.db` เป็นหลัก ใช้ `metadata.opf` เป็นตัวสำรอง** — เดิมเลือก opf
เพราะไม่อยากโหลด sql.js (~1 MB wasm) แต่ทดสอบกับไลบรารีจริงแล้วพบว่าการแลกนั้นคิดผิด

`metadata.opf` เป็น **ภาพนิ่งตอนเพิ่มหนังสือเข้าไลบรารี** ไม่ใช่ข้อมูลปัจจุบัน
เวลาผู้ใช้แก้ series / tags / ผู้เขียนในโปรแกรม Calibre มันเขียนลง `metadata.db` อย่างเดียว
ไฟล์ `.opf` ไม่ถูกแตะ กดสแกนใหม่ในเว็บกี่รอบก็อ่านเจอข้อมูลเก่าเหมือนเดิม

หลักฐานจากไลบรารีจริง 211 เล่ม:

| อาการ | สิ่งที่อยู่ในไฟล์ `.opf` บน Drive |
|---|---|
| เจาะเวลาหาจิ๋นซี เล่ม 1–6 รวมเป็นชุดได้ แต่เลขเล่มเป็น 1 หมด | ทุกใบเขียน `series_index="1"` |
| 49 เล่มที่ผู้ใช้ตั้ง series แล้วแต่ไม่จับกลุ่ม | ไม่มีคำว่า `calibre:series` อยู่เลย |
| `calibre:timestamp` ในไฟล์ | เป็นวันที่ *เพิ่มหนังสือ* ไม่ใช่วันที่แก้ล่าสุด |

`books.series_index` ใน db เป็นเลขเล่มจริง (float) ซึ่ง `.opf` ให้ไม่ได้เลย

ผลพลอยได้: สแกนเร็วขึ้นมาก จากเดิมต้องโหลด `.opf` 151 ไฟล์ + EPUB เต็มเล่มอีก 59 เล่ม
เหลือโหลด `metadata.db` ก้อนเดียว ส่วน `.opf`/EPUB ใช้เฉพาะเล่มที่ไม่มีใน db
(เช่นโฟลเดอร์ที่ถูกย้ายมาเองโดยไม่ผ่าน Calibre) ถ้าโหลด db ไม่ได้ก็ถอยไปทางเดิมทั้งหมด ไม่ทำให้สแกนพัง

sql.js ดึงจาก CDN ทั้ง js และ wasm ไม่ใส่ใน `package.json` — ถ้าเวอร์ชันของ npm
กับของ wasm หลุดจากกันจะพังแบบเงียบ ๆ ดึงจากที่เดียวกันแล้วจบ
...
5. epub    — เล่มที่ *ไม่มี* metadata.opf ถึงจะเข้าขั้นนี้ (ในไลบรารีจริง 60 จาก 221 เล่ม)
             ดาวน์โหลดไฟล์ EPUB ทั้งเล่ม แล้วแกะ OPF ที่ฝังอยู่ข้างในด้วย parser ตัวเดียวกัน
             ทำทีละ 3 เล่มเท่านั้นเพราะ payload เป็นหลาย MB ไม่ใช่หลาย KB
6. fallback— ถ้าทั้ง opf และ epub ไม่ได้เรื่อง ถอยไปใช้ชื่อโฟลเดอร์ ("Sapiens (142)" -> "Sapiens")
             และชื่อโฟลเดอร์แม่เป็นชื่อผู้เขียน
7. cover   — ใช้ cover.jpg ที่ Calibre วางไว้ให้แล้ว ไม่ต้องแกะจาก EPUB
8. commit  — append เข้า library.json แล้ว sync
```

**ทำไมต้องมีขั้น epub** โฟลเดอร์ที่ไม่มี metadata.opf ทำให้ต้องใช้ชื่อโฟลเดอร์แทน
แต่ Calibre ถอดชื่อไทย/จีนเป็นอักษรโรมันตอนตั้งชื่อโฟลเดอร์ ผลคือได้ชื่อแบบ
`"02 suuwithii`suraa (Qiu Mo )"` ซึ่งอ่านไม่ออกเลย ทั้งที่ชื่อไทยจริงอยู่ใน EPUB มาตลอด

`Book.metaSource` (`'opf' | 'epub' | 'folder'`) บันทึกว่าข้อมูลที่แสดงมาจากไหน
รอบสแกนถัดไปจึงข้ามการดาวน์โหลดซ้ำได้ถ้า `metaSource === 'epub'` และ `modifiedTime`
ของไฟล์ EPUB ไม่เปลี่ยน — ต้นทุนหนักจึงจ่ายครั้งเดียว

**ต้องตัดเล่มที่หายไปจาก Drive ทุกรอบ** ขั้น diff เทียบด้วย Drive `folderId`
ซึ่งจับ "เล่มใหม่" ได้ถูก แต่ไม่เคยจับ "เล่มที่หายไป" เลย ผลคือเกิดเล่มผีค้างถาวร:
ผู้ใช้ลบหนังสือใน Calibre แล้วเพิ่มกลับเข้ามาใหม่ Calibre สร้างโฟลเดอร์ใหม่ให้
(`folderId` คนละตัว แม้ `calibreId` จะเท่ากัน) ระบบจึงมองว่าเป็นคนละเล่มแล้วเพิ่มอีกใบ
ส่วนใบเก่าไม่มีใครลบ เพราะ merge เดิมเก็บ `get().books` ไว้ทั้งหมดโดยไม่เคยตรวจว่ายังมีจริงไหม
...
**กันพลาดก่อนลบ** ถ้ารอบนั้น Drive คืนโฟลเดอร์มาน้อยกว่าครึ่งของจำนวนที่เคยมี
ให้ข้ามขั้นตัดทิ้งไปเลย เพราะน่าจะสแกนไม่ครบ (เน็ตหลุด, token หมดกลางคัน, เลือกโฟลเดอร์ผิด)
...
## 6.4.1 เอียงตามการหมุนเครื่องบนมือถือ (`lib/tilt/orientation.ts`)

บนเดสก์ท็อป `TiltCard` เอียงเฉพาะการ์ดที่เมาส์ชี้อยู่ แต่บนมือถือไม่มี hover
จึงใช้ `deviceorientation` แทน ซึ่งเปลี่ยนโครงสร้างไปคนละแบบ

**ไม่ใส่ listener ต่อการ์ด** เวลาเอียงเครื่อง *ทุกใบเอียงเท่ากันหมด* ต่างจากเมาส์
จึงเขียนมุมลง CSS variable `--drx` / `--dry` บน `<html>` ที่เดียว แล้วให้ CSS
กระจายไปทุกใบผ่าน `.tilt-motion .tilt-inner` ผลคือมี JS ทำงานเฟรมละครั้งเดียว
ไม่ว่าหน้าไลบรารีจะมีการ์ด 10 ใบหรือ 1,000 ใบ ถ้าทำแบบผูก listener ต่อการ์ด
จะได้ 111 callback ต่อ event บนเครื่องที่แรงน้อยที่สุดในบรรดาอุปกรณ์ทั้งหมด

**iOS ต้องขอสิทธิ์จาก user gesture** `DeviceOrientationEvent.requestPermission()`
ใช้ได้เฉพาะเมื่อถูกเรียกจาก event ที่ผู้ใช้กดเอง เรียกใน `useEffect` ตอนโหลดหน้า
จะถูกปฏิเสธทันทีโดยไม่ขึ้นกล่องถามด้วยซ้ำ ปุ่มเปิดจึงต้องอยู่ในหน้าตั้งค่า
และ `TiltMotion` ที่อยู่ใน layout มีหน้าที่แค่ "เปิดตามค่าที่บันทึกไว้" ไม่ขอสิทธิ์เอง

**สองอย่างที่ต้องมี ไม่งั้นใช้ไม่ได้จริง**

1. *จุดศูนย์ตามท่าถือ* — คนอ่านมือถือถือเอียงเข้าหาตัวราว 40–60° ถ้าวัดจากแนวราบ
   การ์ดจะเอียงค้างสุดตั้งแต่ยังไม่ขยับเครื่อง จึงจับค่าแรกที่อ่านได้เป็นศูนย์
   และตั้งใหม่ทุกครั้งที่หมุนจอหรือสลับกลับมาจากแอปอื่น
2. *เกลี่ยค่า* — ค่าดิบจากเซนเซอร์สั่นตลอดเวลา ถ้าใช้ตรง ๆ การ์ดจะกระตุกแม้วางเครื่อง
   นิ่งบนโต๊ะ จึงวิ่งเข้าหาเป้าหมายทีละ 18% ต่อเฟรม แล้วหยุด rAF เมื่อนิ่งพอ
   (ไม่กิน CPU ตอนไม่ได้ขยับ)

`beta` ข้ามเส้น ±180 ตอนพลิกเครื่องแล้วกระโดดทั้งรอบ ต้องดึงส่วนต่างกลับเข้าช่วง
-180..180 ก่อนเสมอ ไม่งั้นการ์ดจะสะบัดหนึ่งครั้งทุกรอบที่พลิก
...
## 6.5.1 การเรียงในหน้าไลบรารี — กับดักที่เจอจริง

หน้าไลบรารีเรนเดอร์จาก `grouped()` ไม่ใช่ `filtered()` เพราะต้องยุบชุดหนังสือเป็นการ์ดเดียว
**การยุบทำลายลำดับที่ `filtered()` เรียงมาแล้ว** (เล่มเดี่ยวถูก push ก่อนชุดเสมอ)
`grouped()` จึงต้องเรียงใหม่เองด้วย `entryCmp(sort)` — เดิมโค้ดเรียงทับด้วยชื่อเรื่องตายตัว
ผลคือเมนู "เรียงตาม…" กดแล้วไม่มีอะไรเปลี่ยนเลยทุกตัวเลือก

ชุดหนังสือมีหลายเล่ม จึงต้องยุบค่าให้เหลือค่าเดียวก่อนเทียบ:

| เรียงตาม | ค่าของชุดหนังสือ | เหตุผล |
|---|---|---|
| เพิ่มล่าสุด / เปิดอ่านล่าสุด | ค่า **มากสุด** ในชุด | เล่มไหนขยับ ถือว่าทั้งชุดขยับ |
| ความคืบหน้า | ค่า **เฉลี่ย** ในชุด | อ่านจบ 1 จาก 20 เล่มไม่ใช่ชุดที่ใกล้จบ ถ้าใช้ max จะเด้งขึ้นหัว 100% |
| ผู้เขียน | ผู้เขียนของเล่มแรก | ทั้งชุดคนเขียนคนเดียวกันอยู่แล้ว |

**`addedAt` ต้องมาจาก Drive ไม่ใช่เวลาที่กดสแกน** ตอนแรกเซ็ต `addedAt = now` ตอน ingest
ผลคือหนังสือทุกเล่มได้ timestamp เดียวกันเป๊ะ (ทดสอบจริง: 221 เล่ม ค่าไม่ซ้ำ = **1**)
"เรียงตามเพิ่มล่าสุด" จึงตกไปที่ tie-breaker คือชื่อเรื่องทุกครั้ง ดูเหมือนปุ่มเสีย
เปลี่ยนมาใช้ `modifiedTime` มากสุดของไฟล์ในเล่มแทน → ค่าไม่ซ้ำ 221/221 กระจายตั้งแต่ 2024-08 ถึง 2026-07

หมายเหตุ: "เปิดอ่านล่าสุด" กับ "ความคืบหน้า" จะยังดูเหมือนไม่ทำงานจนกว่าจะมีประวัติการอ่านจริง
เพราะทั้งไลบรารีมี `lastOpenedAt` = 0 เล่ม และ `percent > 0` = 0 เล่ม นี่คือข้อมูลว่าง ไม่ใช่บั๊ก

## 6.5.2 หน้ารายละเอียดหนังสือ (`components/library/BookSheet.tsx`)

คลิกการ์ดแล้วเปิดรายละเอียดก่อน ไม่กระโดดเข้าโปรแกรมอ่านทันที
เพราะเล่มที่มีหลายฟอร์แมตต้องเลือกก่อน และคนมักอยากอ่านเรื่องย่อก่อนตัดสินใจ

**จอใหญ่เป็น modal / มือถือเป็น full-screen sheet แต่ใช้ DOM ชุดเดียวกัน**
ต่างกันแค่ Tailwind breakpoint ไม่มี branch ใน JS — ถ้าเช็ค `window.innerWidth`
ตอน render จะได้ hydration mismatch เพราะ server ไม่รู้ขนาดจอ

**ต้องใช้ `createPortal` ไปที่ `document.body`** การ์ดหนังสืออยู่ใน `TiltCard`
ซึ่งมี `transform` และ `transform` ทำให้ `position: fixed` ยึดกับ element นั้น
แทนที่จะยึดกับ viewport ถ้าเรนเดอร์ตรงที่ modal จะไปโผล่ในกรอบการ์ดใบเล็ก ๆ แทนที่จะเต็มจอ

**การ์ดเปลี่ยนจาก `<Link>` เป็น `<button>`** เพราะปลายทางเป็น modal ไม่ใช่ URL
ต้องใส่ `w-full text-left` เอง ไม่งั้นข้อความใต้ปกจะถูกจัดกึ่งกลางตาม default ของปุ่ม
และ sheet ต้องเรนเดอร์นอกปุ่ม (เป็น sibling ใน fragment) เพราะข้างใน sheet มีปุ่มอีกหลายตัว
ซ้อน `<button>` ใน `<button>` เป็น HTML ที่ไม่ถูกต้อง

**เรื่องย่อล้าง HTML tag ทิ้งด้วย regex ไม่ใช้ `dangerouslySetInnerHTML`**
`metadata.opf` ของ Calibre ใส่ `<p>` / `<br>` มาด้วย แต่ไฟล์นั้นมาจากไลบรารีของผู้ใช้
ซึ่งอาจโหลดมาจากที่ไหนก็ได้ จึงไม่ควรเชื่อว่าปลอดภัย

ทางเข้าโปรแกรมอ่านโดยตรงยังเหลือไว้ที่แถว "อ่านต่อ" และห้อง 3 มิติ —
สองที่นั้นผู้ใช้ตัดสินใจแล้วว่าจะอ่านเล่มไหน การขวางด้วยรายละเอียดอีกชั้นมีแต่จะกวน

## 5.1 กับดัก Cache-Control ของ `/api/drive/file/[id]`

route นี้เคยส่ง `Cache-Control: private, max-age=31536000, immutable`
โดยให้เหตุผลว่า "Drive file id คงที่ตลอดอายุไฟล์ แคชยาวได้"
ข้อสังเกตถูก แต่ข้อสรุปผิด — **id คงที่ก็จริง แต่เนื้อไฟล์เปลี่ยนได้โดยที่ id ไม่เปลี่ยน**

`immutable` มีความหมายชัดเจนว่า *ห้าม revalidate เลยแม้แต่ครั้งเดียว*
`metadata.db` จึงถูกแช่อยู่ในแคชของเบราว์เซอร์ตลอดกาล ผู้ใช้แก้ข้อมูลใน Calibre
กดสแกนใหม่กี่รอบก็ได้ไฟล์เดิมกลับมา และไม่มีอะไรในหน้าเว็บบอกว่ากำลังอ่านของเก่าอยู่

พิสูจน์บนเครื่องจริง ยิงไฟล์ id เดียวกันสองแบบ:

```
fetch(url)                    -> 1,568,768 bytes  sha256 e7fcefd3...
fetch(url, {cache:'no-store'}) -> 1,540,096 bytes  sha256 ab2441a1...
```

**ที่แก้**

1. ตัด `immutable` ออก ลดเหลือ 30 วัน — ปกและไฟล์หนังสือยังได้ประโยชน์จากแคชเต็มที่
2. ฝั่ง client เติม `?t={modifiedTime}` ต่อท้าย URL ของ `metadata.db`
   แก้ไฟล์ใน Calibre เมื่อไหร่ค่านี้ขยับ กลายเป็นคนละ cache key ทันที
3. ใส่ `cache: 'no-store'` ซ้ำอีกชั้น กัน HTTP cache ระหว่างทางที่ไม่สนใจ query string

บทเรียนที่ใช้ซ้ำได้: `immutable` ใช้ได้เฉพาะกับ URL ที่ผูกกับ *เนื้อหา* (เช่นชื่อไฟล์มี hash
แบบ `/_next/static/`) ไม่ใช่ URL ที่ผูกกับ *ตัวตน* ของทรัพยากรอย่าง Drive file id
...
## 6.5.2.0 ฟอนต์ในตัว — เดิมไม่เคยถูกโหลดเลย

`FONT_STACK` เดิมอ้างชื่อ `'Lora'` / `'Inter'` / `'Noto Serif Thai'` /
`'Atkinson Hyperlegible'` ไว้เฉย ๆ **โดยที่แอปไม่เคยโหลด webfont เลยสักตัว**
(`grep` หา `next/font`, `fonts.googleapis`, `@font-face` ในโปรเจกต์ได้ผลว่าง)
ชื่อพวกนั้นจึงไม่ชี้ไปไหน ตกไปใช้ Georgia / system-ui หมด
สามตัวเลือกเดิมเลยให้ผลหน้าตาเหมือนกันทั้งสามอัน

ตอนนี้มีฟอนต์ในตัว 12 ตัว **คัดเฉพาะที่มีอักขระไทยครบ** เพราะไลบรารีของผู้ใช้
เป็นภาษาไทย 215 จาก 221 เล่ม แบ่งเป็น ไม่มีหัว / มีเชิง / มีหัว / ช่วยการอ่าน
ทุกตัวถูกยิงทดสอบกับ `fonts.googleapis.com` แล้วว่าคืน 200 และมี subset ไทยจริง
(ตรวจทีละตัวด้วย ไม่ใช่แค่ URL รวม — ถ้าตัวใดตัวหนึ่งผิด API จะคืน 400 ทั้งก้อน
แล้วฟอนต์พังหมดพร้อมกัน)

**ต้องแปะ `<link>` ซ้ำใน iframe** `<link>` ที่อยู่ใน `app/layout.tsx` ไม่มีผลกับ
เนื้อหนังสือ เพราะ epub.js วาดใน iframe ที่เป็นคนละ document — เรื่องเดียวกับ
`document.fonts.add()` ที่ไม่ข้ามไป ใช้ `<link>` ไม่ใช่ `@import` เพราะ `@import`
ต้องเป็นกฎแรกของ stylesheet ซึ่งชนกับ `@font-face` ของฟอนต์ที่ผู้ใช้เพิ่มเอง
ที่ต่ออยู่ในก้อนเดียวกัน

**`fontCatalog.ts` ตั้งใจไม่ใส่ `'use client'`** — `app/layout.tsx` เป็น server
component และต้องใช้ `GOOGLE_FONTS_HREF` เป็นสตริงจริง ถ้า import มาจากโมดูล
ที่ประกาศ `'use client'` ค่าที่ได้จะเป็น client reference ไม่ใช่สตริง แล้ว
`<link href>` จะพัง `prefs.ts` จึง re-export ต่อจากที่นี่แทน

## 6.5.2.1 ทำไมไม่ใช้ `rendition.themes` ของ epub.js

**ธีมของหน้าอ่านไม่เคยถูกใช้เลยตั้งแต่แรก** ไม่ใช่แค่ฟอนต์ — สี ระยะบรรทัด การจัดขอบ
ทั้งหมดถูกเขียนผ่าน `themes.register('bd', {...})` + `themes.select('bd')`
ซึ่งบนเครื่องจริงสร้าง `<style id="epubjs-inserted-css-">` ขึ้นมา **ว่างเปล่า**

ตรวจจากใน iframe ของหน้าที่ deploy แล้ว:

```
head ของ iframe : [..., STYLE#epubjs-inserted-css-]   <- tag ถูกสร้าง
เนื้อใน style    : ""                                  <- แต่ไม่มีอะไรข้างใน
computed font   : "Times New Roman"                    <- ค่าเริ่มต้นของเบราว์เซอร์
กฎที่ตั้ง font-family ทั้งเอกสาร : []                    <- ไม่มีเลยสักกฎ
ทดลองฉีด <style> เอง -> ได้ผลทันที
```

รูปแบบข้อมูลที่ `themes.register()` รับ กับที่ epub.js รุ่นนี้เขียนออกมาไม่ตรงกัน
แทนที่จะไล่หาว่ารุ่นไหนต้องส่งรูปแบบไหน เลิกใช้กลไกนี้แล้วเขียน `<style id="bd-style">`
ลงในเอกสารของ iframe เอง — คุมได้หมดและไม่ผูกกับรายละเอียดภายในของไลบรารี

**ต้องระบุ selector ให้ครบ ไม่ใช่แค่ `body`** EPUB จำนวนมากตั้ง `font-family` ไว้ที่
`p` / `div` / `span` ของตัวเอง ซึ่งชนะกฎที่ `body` เสมอ **ต่อให้ `body` ใส่ `!important`
ก็ตาม** เพราะเป็นคนละ element ไม่ใช่เรื่องการสืบทอด — `!important` ที่ `body`
ชนะได้เฉพาะกับ element ที่ *ไม่มี* กฎของตัวเองเท่านั้น

`code` / `pre` แยกออกมาใช้ monospace เสมอ ไม่งั้นตารางและผังในหนังสือเทคนิคจะเพี้ยน

## 6.5.3 ฟอนต์ที่ผู้ใช้เพิ่มเอง (`lib/reader/fonts.ts`)

รับ `.woff2 .woff .ttf .otf` เก็บเป็น Blob ใน IndexedDB ตาราง `fonts` (Dexie version 2)

**ไม่ส่งขึ้น Drive โดยตั้งใจ** เป็นไฟล์ของผู้ใช้ที่เรามีสิทธิ์แค่ "ใช้แสดงผลให้เขาดู"
ไม่ใช่ "เอาไปเก็บไว้ที่อื่น" และฟอนต์เกือบทั้งหมดมีสัญญาอนุญาตที่ห้ามแจกจ่ายซ้ำ
ค่าตั้งอื่นของหน้าอ่านก็เป็นของเครื่องอยู่แล้ว (localStorage) จึงสอดคล้องกัน

**กับดักใหญ่: `document.fonts.add()` ไปไม่ถึงเนื้อหนังสือ**
epub.js วาดเนื้อหาใน iframe ซึ่งมี `document` คนละตัวกับหน้าหลัก ฟอนต์ที่ลงทะเบียน
ไว้ที่หน้าหลักจึงใช้ในนั้นไม่ได้เลย ต้องแปะ `@font-face` (ชี้ไป blob URL)
ลงในเอกสารของ iframe เอง ผ่าน `rendition.hooks.content.register()`
และ **ต้องแปะทุกครั้งที่มีหน้าใหม่ถูกเรนเดอร์** เพราะ epub.js สร้าง document ใหม่
ทุกครั้งที่ข้ามบท ไม่ใช่ครั้งเดียวจบ ส่วนหน้าที่เรนเดอร์ไปแล้วต้องไล่ `getContents()`
ฉีดซ้ำเอง ไม่งั้นเพิ่มฟอนต์ระหว่างอ่านอยู่จะไม่เห็นผลจนกว่าจะข้ามบท

**อีกสองอย่างที่ต้องมี**

- *ทดลองโหลดก่อนบันทึก* — `new FontFace().load()` ให้ครั้งหนึ่งก่อนเขียนลง db
  ไฟล์เสียหรือไม่ใช่ฟอนต์จะพังตรงนั้นพร้อมข้อความบอกเหตุ ดีกว่าบันทึกสำเร็จ
  แล้วไปเงียบตอนอ่านหนังสือโดยไม่มีใครรู้ว่าทำไมตัวหนังสือไม่เปลี่ยน
- *fallback ภาษาไทย* — ฟอนต์ละตินหลายตัวไม่มีสระไทย ถ้าไม่ต่อ `Noto Sans Thai`
  ท้าย stack ข้อความไทยจะกลายเป็นกล่องว่างทั้งเล่ม (`fontStackFor()`)

blob URL ถูก cache ไว้ใน `Map` ต่อฟอนต์หนึ่งตัว — สร้างใหม่ทุกครั้งที่เรนเดอร์
จะรั่วเรื่อย ๆ เพราะ blob URL อยู่จนกว่าจะ revoke หรือปิดแท็บ

## 6.6.1 หน้าอ่านแยกกลุ่ม route ออกมาต่างหาก

`/read/[id]` ย้ายจาก `app/(app)/` ไป `app/(reader)/` — URL เหมือนเดิมทุกอย่าง
แต่ไม่ต้องแบก layout ของไลบรารีมาด้วย

เดิมหน้าอ่านอยู่ใต้ `(app)` ซึ่งมี `AppNav` เป็นแถบ fixed บนสุดของจอมือถือ
บวก `pt-14` ที่กันที่ให้แถบนั้นอีกชั้น ผลคือบนมือถือเสียพื้นที่อ่านไปฟรี ๆ
และแถบเมนูค้างทับอยู่ตลอด แม้ผู้ใช้จะซ่อน chrome ของหน้าอ่านไปแล้วก็ตาม

**`100dvh` ไม่ใช่ `100vh`** บน iOS Safari ค่า `100vh` นับรวมพื้นที่ใต้แถบ URL
ที่ยุบ/ขยายตอนเลื่อน แถบความคืบหน้าด้านล่างจึงถูกดันตกขอบจอหายไปเลย
`dvh` วัดจากพื้นที่ที่มองเห็นจริงในขณะนั้น

**`env(safe-area-inset-*)` ต้องใส่ให้ครบทุกตัวที่ยึดขอบจอ ไม่ใช่แค่แถบหลัก**
รอบแรกใส่ให้เฉพาะแถบบน/ล่างของหน้าอ่าน แต่ลืมแผงข้าง (สารบัญ/ค้นหา/การแสดงผล)
ซึ่งเป็น `fixed top-0` เหมือนกัน ผลคือบนมือถือปุ่มปิดของแผงไปอยู่ *ใต้รอยบาก*
กดไม่โดน กลายเป็นเข้าแผงตั้งค่าแล้วออกไม่ได้เลย ต้องปิดแท็บทิ้งอย่างเดียว
บทเรียน: ทุก element ที่ `fixed` ชิดขอบจอต้องถูกตรวจ ไม่ใช่แค่ตัวที่นึกถึงตอนนั้น

แผงยังใช้ `h-[100dvh]` ไม่ใช่ `h-full` ด้วยเหตุผลเดียวกับตัวหน้า
และปุ่มปิดขยายเป็น 44×44px ซึ่งเป็นขนาดขั้นต่ำที่นิ้วกดติด ของเดิมเป็นตัวอักษร `✕`
ที่มี padding แค่ 8px ซ้ายขวา

**`env(safe-area-inset-*)`** แถบบนต้องเผื่อรอยบาก แถบล่างต้องเผื่อแถบ home indicator
ไม่งั้นปุ่มย้อนกลับจะไปอยู่ใต้รอยบากและเปอร์เซ็นต์จะทับเส้นขีดล่างของ iPhone
(`viewportFit: 'cover'` ตั้งไว้ใน root layout อยู่แล้ว ถ้าไม่มี ค่า env จะเป็น 0 หมด)
...
เทียบกับ BookFusion ตัวจริงแล้วปรับใหม่ทั้งชุด — ของเดิมเป็นแถบข้างกรมท่าเข้ม
กับเขียวมิ้นต์ ซึ่งคนละภาษากับตัวจริงที่เป็นพื้นสว่างเกือบขาวตัดด้วยน้ำเงินสด

```css
--color-brand:  #2f6bff;   /* ปุ่มหลัก เมนูที่เลือก ชิปที่เปิดอยู่ */
--color-mint:   #22c55e;   /* "อ่านต่อ" */
--color-amber:  #ffc24b;   /* แถบความคืบหน้าใต้ปก */
--color-shell:  #f7f8fc;   /* พื้นหลัง */
--color-navy:   #1b1f2e;   /* เมนูลอย ป้ายเข้ม หน้าล็อกอิน — ไม่ใช่แถบข้างแล้ว */
```

**บทเรียนตอนเปลี่ยนพาเลต** เคยเผลอตั้ง `--color-navy` เป็นสีขาวเพราะแถบข้างเปลี่ยนเป็นขาว
แต่ยังมีอีกหกที่ใช้ `bg-navy text-white` อยู่ กลายเป็นขาวบนขาวมองไม่เห็นทันที
ถ้าความหมายของตัวแปรเปลี่ยน ต้องไล่ทุกที่ที่ใช้ ไม่ใช่แค่เปลี่ยนค่า

**สิ่งที่ยกมาจาก BookFusion**

- แถบข้างขาวมีวงแหวนสรุปการอ่านด้านบน และตัวเลขจำนวนเล่มข้างทุกเมนู
- **แถว "อ่านต่อ" อยู่บนสุด** เป็นการ์ดแนวนอน ปกซ้าย ข้อความขวา พร้อมวงแหวนเปอร์เซ็นต์
  วางไว้ก่อนทุกอย่างเพราะคนเปิดแอปมาเพื่ออ่านต่อ ไม่ใช่มาไล่ดูทั้งชั้น
  ซ่อนเมื่อกำลังกรองอยู่ เพราะตอนนั้นผู้ใช้กำลังหาอย่างอื่น
- **การ์ด: ผู้เขียนอยู่บน ชื่อเรื่องอยู่ล่าง** และแถบความคืบหน้าย้ายมาอยู่ใต้ปกแทนที่จะทับบนปก
  ทำให้กวาดตาดูทั้งชั้นรอบเดียวรู้ว่าค้างเล่มไหนไว้บ้าง
