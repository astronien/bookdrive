  ...
  return (
    <Link href={`/read/${book.id}`} className="group block">
      <div className="relative aspect-[2/3] overflow-hidden rounded-[9px] shadow-[0_4px_16px_rgba(25,29,68,.10)] transition duration-200 group-hover:-translate-y-1 group-hover:shadow-[0_12px_40px_rgba(25,29,68,.20)]">
        ...
        <span className="absolute right-2 top-2 rounded bg-black/50 px-1.5 py-0.5 text-[8.5px] font-bold tracking-wider text-white backdrop-blur">
      ...
      <div className="pt-2.5">
        ...
        {book.percent > 0 && (
          <div className="mt-0.5 text-[10.5px] font-semibold text-accent-d">อ่านแล้ว {Math.round(book.percent)}%</div>
        )}
      </div>
    </Link>
  );
